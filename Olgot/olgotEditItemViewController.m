...
@interface olgotEditItemViewController ()

@end


@implementation olgotEditItemViewController
...
@synthesize itemToView = _itemToView;
        ...
        if ([response isJSON]) {
            ...
            if([response isOK]){
                if ([[request userData] isEqual:@"postItem"]) {
                    ...
                    self.itemToView.itemID = [_itemJsonResponse objectForKey:@"id"];
                    self.itemToView.itemKey = [_itemJsonResponse objectForKey:@"key"];
                    self.itemToView.itemUrl = [_itemJsonResponse objectForKey:@"itemUrl"];
...
-(void)fillUi
{
    _venue = [[olgotVenue alloc] init];
    self.itemNameTF.text = [self.itemToView itemName];
    self.itemPriceTF.text = [self.itemToView itemPrice];
    [self.itemImageView setImageWithURL:[NSURL URLWithString:[self.itemToView itemPhotoUrl]]];
    _itemImage = self.itemImageView.image;
    _venue.venueId = [self.itemToView venueId];
    _venue.name_En = [self.itemToView venueName_En];
    self.whereTxtField.text = _venue.name_En;//[self.itemToView venueName_En];
    self.descriptionTF.text = [self.itemToView itemDescription];
    
    
 
    if([self.itemPriceTF.text isEqualToString:@""])
    {
    ...
    [accountStore requestAccessToAccountsWithType:accountType withCompletionHandler:^(BOOL granted, NSError *error) {
        if(granted) {
            ...
            if ([accountsArray count] > 0) {
                ...
                NSString *status = [NSString stringWithFormat:@"Just posted this  at %@ using #Olgot %@ %@", [_venue name_En], venueTwitterName,self.itemToView.itemUrl];
...
-(void)showAddItemConfirmation
{
    ...
    confirmationController.itemID = self.itemToView.itemID;
    confirmationController.itemKey = self.itemToView.itemKey;
    confirmationController.itemUrl = self.itemToView.itemUrl;
    ...
    [params setValue:self.itemToView.itemID forParam:@"item"];
    ...
    confirmationController.editDelegate = self.editDelegate;//[self.navigationController.viewControllers objectAtIndex:self.navigationController.viewControllers.count-1-1];
...
-(void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    if([[segue identifier] isEqual:@"ShowAddItemConfirmation"]){
        ...
        confirmationController.venueName = [_venue name_En];
        confirmationController.venueItemCount = [_venue items];
        confirmationController.editDelegate = self.editDelegate;
